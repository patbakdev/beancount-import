"""Fidelity.com brokerage transaction source.

Imports transactions from Fidelity.com brokerage/banking history CSV files.

To use, first you have to download Fidelity CSV data into a directory on your filesystem. If
...
this:
    ...
    dict(module="beancount_import.source.fidelity_csv",
         transaction_csv_filenames=glob.glob("data/fidelity/transactions/*.csv"),
         position_csv_filenames=glob.glob("data/fidelity/positions/*.csv"),
    )

The importer can also optionally make use of Fidelity's lot details CSV downloads in order
...
downloaded and one file per commodity. So your source spec might look like this:
    ...
    dict(module="beancount_import.source.fidelity_csv",
         transaction_csv_filenames=glob.glob("data/fidelity/transactions/*.csv"),
         position_csv_filenames=glob.glob("data/fidelity/positions/*.csv"),
         lots_csv_filenames=glob.glob("data/fidelity/positions/lots/*/*.csv"),
    )

This importer also makes use of certain metadata keys on your accounts. In order to label
a beancount account as a Fidelity account whose authoritative transaction source is this
importer, specify the `account_id` metadata key as the account ID exactly as it
appears in your Fidelity CSV downloads. For example:

    2015-11-09 open Assets:Investments:Fidelity:Brokerage-1234
         account_id: "XXXX-1234"
...
gains, interest, fees, and taxes:
    ...
    2015-11-09 open Assets:Investments:Fidelity:Brokerage-1234
         account_id: "XXXX-1234"
         div_income_account: "Income:Dividend:Fidelity"
         interest_income_account: "Income:Interest:Fidelity"
         capital_gains_account: "Income:Capital-Gains:Fidelity"
         fees_account: "Expenses:Brokerage-Fees:Fidelity"
         taxes_account: "Expenses:Taxes"

These are all optional and will fall back to `Expenses:FIXME` if not specified.

This importer will add the metadata keys `date`, `source_desc`, and `fidelity_action` to the
...
security as needed; e.g. `Assets:Investments:Fidelity:Brokerage-1234:XYZ` would be created
to track the balance of `XYZ` shares owned, and `Income:Dividend:Fidelity:XYZ` for dividends
earned from `XYZ`, etc.

Caveats
=======

* Because Fidelity CSV downloads do not provide any unique transaction identifier, and it is
...
* Not all Fidelity "actions" (transaction types) are supported. There's no reference for all
the possible actions, and Fidelity could add new ones any time. If your CSV includes an
...
class RawBrokerageEntry(RawEntry):
    ...
    def get_processed_entry(
        ...
        fidelity_account = get_fidelity_account_from_meta(account_meta)
        amount = self.amount
        ...
        if self.action == BrokerageAction.STOCK_PLAN_ACTIVITY:
            cost = lots.get_cost(fidelity_account, self.symbol, self.date)
        ...
        if self.action in (BrokerageAction.REVERSE_SPLIT, BrokerageAction.STOCK_SPLIT):
            assert self.quantity is not None
            lot_splits = lots.split(fidelity_account, self.symbol, self.date, self.quantity)
            return StockSplit(
        ...
        if self.action in (BrokerageAction.SELL,
            ...
            lot_info = lots.get_sale_lots(fidelity_account, self.symbol, self.date, quantity)
            return Sell(
        ...
        if self.action == BrokerageAction.EXPIRED:
            ...
            lot_info = lots.get_sale_lots(fidelity_account, self.symbol, self.date, self.quantity)
            if self.quantity > 0:
...
class EntryProcessor:
    def __init__(self, journal: JournalEditor, lots: LotsDB) -> None:
        (
            self.account_to_fidelity,
            self.fidelity_to_account,
    ...
    def process_entry(self, raw_entry: RawEntry) -> Optional[TransactionEntry]:
        account = self.fidelity_to_account.get(raw_entry.account)
        if account is None:
    ...
    def process_positions(
        ...
        for raw_position in raw_positions:
            account = self.fidelity_to_account.get(raw_position.account)
            if account is None:
...
POSTING_META_ACTION_KEY = "fidelity_action"
POSTING_META_ACCOUNT_KEY = "account_id"
...
def get_fidelity_account_from_meta(account_meta: Mapping[str, str]) -> str:
    raw_acct = account_meta[POSTING_META_ACCOUNT_KEY]
    # Because Fidelity CSVs sometimes list accounts like "Foobar XXXX-1234" and
...
class FidelitySourceSpecDict(TypedDict):
...
def load(spec: FidelitySourceSpecDict, log_status: LogStatusCallable):
    spec.setdefault("lots_csv_filenames", [])
    return FidelitySource(**spec, log_status=log_status)


class FidelitySource(DescriptionBasedSource):
    name = "fidelity"
    ...
    def prepare(self, journal: JournalEditor, results: SourceResults) -> None:
        ...
        account_set = set(processor.account_to_fidelity.keys())
        ...
        for acct in processor.missing_accounts:
            results.add_warning(
                f"No Beancount account associated with Fidelity account {acct}."
            )
    ...
    def _get_pending_and_invalid_entries(
        ...
        for source_entry in source_entries:
            ...
            if not matched:
                ...
                results.add_error(
                    f"Fidelity CSV entry at {source_entry.line} of {source_entry.filename} "
...
class LotsDB:
    """In-memory database of historical lot information from Fidelity lot details CSVs.
...
class HoldingLotsDB:
    ...
    def __init__(self, account: str, symbol: str) -> None:
        ...
        self.symbol = symbol
        # We uniquely identify a lot by (opened, cost). (Fidelity will track separate
...
POSITIONS_DATE_RE = re.compile(r'"Date downloaded (?P<datetime>[0-9/]+).+"')
...
def _load_positions(filename: str) -> Sequence[RawPosition]:
    ...
    with open(filename, "r", encoding="utf-8", newline="") as csvfile:
        for row in csvfile:
            pass
        date = _convert_date(POSITIONS_DATE_RE.match(row).group(1))
    ...
    with open(filename, "r", encoding="utf-8", newline="") as csvfile:
        ...
        for lno, row in enumerate(reader):
            account = _convert_account(row["Account Name/Number"])
            if not account[-1].isdigit():
                # End of Data. The rest is legalize.
                break
            ...
            price_d = _convert_decimal(row["Last Price"])
            price = None if price_d is None else Amount(price_d, currency=CASH_CURRENCY)
            value_d = _convert_decimal(row["Current Value"])
            assert value_d is not None, row["Current Value"]
...
def _convert_account(raw: str) -> str:
    return raw.strip().split(" ")[-1]

def _convert_action(raw: str) -> Union[BrokerageAction, BankingEntryType]:
    first, second = itemgetter(0,1)(raw.strip().split(" "))
    if first == "DIRECT":
        if second == "DEBIT":
            return BankingEntryType.CHECK
    if first == "CASH":
        if second == "ADVANCE":
            return BankingEntryType.ATM
    if first == "ADJUST":
        if second == "FEE":
            return BankingEntryType.ATMREBATE
    if first == "YOU":
        if second == "BOUGHT":
    ...
    if first == "REINVESTMENT":
        return BrokerageAction.BUY
    if first == "CONV.":
        return BrokerageAction.SELL # FIX
    if second == "CONVERSION":
        return BrokerageAction.BUY # FIX
    return BrokerageAction.UNKNOWN
...
def _load_transactions(filename: str) -> List[RawEntry]:
    ...
    with open(filename, "r", encoding="utf-8", newline="") as csvfile:
        ...
        for lno, row in enumerate(reader):
            date = None
            try:
                date = datetime.datetime.strptime(row["Run Date"].strip(), DATE_FORMAT).date()
            except ValueError:
                # End of Data. The rest is legalize.
                break

            account = _convert_account(row["Account"])
            action = _convert_action(row["Action"])
            description = row["Action"].strip()
            symbol = row["Symbol"].strip()
            quantity = _convert_decimal(row["Quantity"])
            price = _convert_decimal(row["Price"])
            # commission = _convert_decimal(row["Commission"])
            fees = _convert_decimal(row["Fees"])
            amount = _convert_decimal(row["Amount"])
            # if OPTION_RE.match(row["Symbol"]) and quantity:
            #     # this is an option, sold in lots of 100
            #     quantity *= 100
            if isinstance(action, BankingEntryType):
                entries.append(
                    RawBankEntry(
                        account=account,
                        date=date,
                        ...
                        amount=Amount(amount, currency=CASH_CURRENCY),
                        running_balance=0,
                        filename=filename,
                        line=lno + 4
                    )
                )
            if isinstance(action, BrokerageAction):
